...
import { StyleSheet,Alert, Text, View,FlatList,ActivityIndicator,Dimensions,Image, ScrollView,TouchableOpacity } from 'react-native';
...
const CardItemMovie = (props) =>{
    ...
    const onAddAndDeleteListInMovie = async () => {
        ...
        if(valid === false)
        {
            ...
            setvalid(true)
            Alert.alert("The movie is Delete")
        }
        else{
            let obj = {
                "ID" : props.ID,
                        ...
                        <View style={styles.informationMovie} >
                            <View style={styles.titleAndBtnAdd}>
                                ...
                                <TouchableOpacity onPress={onAddAndDeleteListInMovie} style={styles.buttonMovie}>
                                    <Text style={styles.btnAdd}>{valid ? <Ionicons name="add-circle-outline" size={24} color="white" /> : <AntDesign name="check" size={24} color="yellow" /> }</Text>
